...
# \<\<\<\<\<\< SEPERATE FUNC AND NON-FUNC T.T
...
```{r}
projection(kde_wpNGA_owin_bw_raster) <- CRS("+init=EPSG:26392 +units=km" )
kde_wpNGA_owin_bw_raster
```
...
```{r}
wp_oe_ppp = wp_ppp_osun[oe_owin]
wp_ow_ppp = wp_ppp_osun[ow_owin]
wp_oc_ppp = wp_ppp_osun[oc_owin]
```

Next, rescale() function is used to transform the unit of measurement from m to km

```{r}
wp_oe_ppp.km = rescale(wp_oe_ppp, 1000, "km")
wp_ow_ppp.km = rescale(wp_ow_ppp, 1000, "km")
wp_oc_ppp.km = rescale(wp_oc_ppp, 1000, "km")
```
...
```{r}
par(mfrow=c(1,3))
plot(wp_oe_ppp.km, main="Osun East")
plot(wp_ow_ppp.km, main="Osun West")
plot(wp_oc_ppp.km, main="Osun Central")
```
...
```{r}
par(mfrow=c(1,3))
plot(density(wp_oe_ppp.km, 
             sigma=bw.diggle, 
             edge=TRUE, 
             kernel="gaussian"),
     main="Osun East")
plot(density(wp_ow_ppp.km, 
             sigma=bw.diggle, 
             edge=TRUE, 
             kernel="gaussian"),
     main="Osun West")
plot(density(wp_oc_ppp.km, 
             sigma=bw.diggle, 
             edge=TRUE, 
             kernel="gaussian"),
     main="Osun Central")
...
Based on the 3 graph above, we can see that central area seems more concentrated. Lets compare point map and kernel density map fro Osun Central area.
