...
-   for further EDA, we can include the column of **quality**

-   need check the CRS for the map (resoning for the dots that lies outside of the region, we uses owin to determine.)

    Polygon data may contain geometrical inconsistencies such as self-intersections and overlaps. These inconsistencies must be removed to prevent problems in other spatstat functions. By default, polygon data will be repaired automatically using polygon-clipping code. The repair process may change the number of vertices in a polygon and the number of polygon components.

-   unique state

```{r}
unique(wp_osun_nga$`#clean_adm2`)
```
